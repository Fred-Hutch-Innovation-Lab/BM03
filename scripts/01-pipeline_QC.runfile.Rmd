...
parse_name <- function(x) {
  x <- as.data.frame(str_match(x, '^([^-]+)(?:-combined)?-(F\\d)(L|S)(A|B)?_?(.+)?'))
  # as.data.frame(str_match(x,      '(^.+-F\\d[^_]*)_+([^_]*)?'))
  colnames(x) <- c(
...
```{r}
data_dir <- here('../processing/mixcr/RNA/downsampled_data/by_reads/')
qc_metrics <- list.files(path = data_dir, pattern = '*QC.txt', recursive = TRUE, full.names = TRUE, ignore.case = FALSE)
names(qc_metrics) <- gsub('.+/(.+-F\\w+)_[qQ][cC].+', '\\1', qc_metrics)
...
```{r}
files <- dir(here('../processing/mixcr/RNA/downsampled_data/by_reads'), 
             'downsampling_summary.*.txt', 
             recursive = TRUE, 
             full.names = TRUE)

# ^([^-]+)(?:-combined)?-(F\\d)(L|S)(A|B)?_?(.+)?
  
downsampling_umis <- lapply(
  files, 
  function(file) {
    sample <- basename(dirname(dirname(file)))# gsub('^([^-]+)(?:-combined)?-(F\\d)', '\\1', file)
    size <- qc_metadata[qc_metadata$sample==sample,'size'] #gsub('.*_summary_([0-9]*).txt', '\\1', file)
