...
```{r, eval=FALSE}
# Step 1: Flatten the chainUsage data with additional 'goodReads' calculation
chain_usage_df <- do.call(rbind, lapply(names(data$chainUsage$chains), function(chain) {
  chain_data <- data$chainUsage$chains[[chain]]
  
  # Calculate goodReads as total - (nonFunctional + isOOF + hasStops)
  goodReads <- chain_data$total - (chain_data$nonFunctional + chain_data$isOOF + chain_data$hasStops)
  
  # Create a dataframe for the chain totals and individual categories
  chain_df <- rbind(
    # data.frame(from = "aligned", to = chain, count = chain_data$total),
    data.frame(chain = chain, final = paste(chain, "goodReads", sep = "_"), count = goodReads),
    data.frame(chain = chain, final = paste(chain, "nonFunctional", sep = "_"), count = chain_data$nonFunctional),
    data.frame(chain = chain, final = paste(chain, "isOOF", sep = "_"), count = chain_data$isOOF),
    data.frame(chain = chain, final = paste(chain, "hasStops", sep = "_"), count = chain_data$hasStops)
  )
  
  chain_df$final <- gsub(paste0("^", chain, "_"), "", chain_df$final)
  chain_df$aligned <- 'Yes'
  return(chain_df)
}))

chain_usage_df$source

# Step 2: Combine this with the previous data (totalReadsProcessed, notAligned, and notAlignedReasons)

# Data for total reads processed, aligned, and not aligned
total_reads_df <- rbind(
  data.frame(from = "totalReadsProcessed", to = "aligned", count = data$aligned),
  data.frame(from = "totalReadsProcessed", to = "notAligned", count = data$notAligned)
)

# Data for not aligned reasons
not_aligned_df <- data.frame(
  aligned = 'No',
  chain = rep("None", length(data$notAlignedReasons)),
  final = names(data$notAlignedReasons),
  count = unlist(data$notAlignedReasons)
)
not_aligned_df
...
# Assuming combined_df is your final dataframe
ggplot(combined_df,
  ...
  labs(title = "Alluvial Plot of T-cell Repertoire Sequencing",
       y = "Count") +
  theme_minimal()
