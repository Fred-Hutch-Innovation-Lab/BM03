...
saveRDS(rep_data_downsampled, here('rds/rep_data_downsampled.rds'))
```
...
# rep_data <- repFilter(rep_data, .method = 'by.meta',
#             list(Sample = exclude('N-F1LB.alignmentsAIRR', 'N-F1LA.alignmentsAIRR')))
saveRDS(rep_data, here('rds/rep_data_full.rds'))
```

```{r}
rep_data <- readRDS(here('rds/rep_data_downsampled.rds'))
# rep_data_large_tratrb <- repFilter(rep_data_downsampled,
#                                    .method = 'by.meta',
#                                    list(chain = include('TRA', 'TRB'),
#                                         size = include('L')))
# 
# rep_data_small_tratrb <- repFilter(rep_data_downsampled,
#                                    .method = 'by.meta',
#                                    list(chain = include('TRA', 'TRB'),
#                                         size = include('S')))
```
...
tmp <- lapply(tmp$data, select, c('CDR3.aa', 'Proportion')) %>%
  rbindlist(idcol='Sample') %>%
  mutate(Sample = str_sub(Sample,1,  -6)) %>%
  filter(grepl('N', Sample)) 
tmp <- tmp %>%
  inner_join(tmp, by='CDR3.aa', suffix=c('_x', '_y'), relationship = 'many-to-many') %>%
  filter(Sample_x != Sample_y) %>%
  filter(max(Proportion_x, Proportion_y) > 0.001) ## minimum 0.1% of one repertoire
figures[['clonal_abundance_scatter_large_TRA']] <- ggplot(tmp, aes(x=Proportion_x, y=Proportion_y)) +
  geom_point(alpha=0.7, size=0.5) +
  # geom_density_2d_filled() +
  facet_grid(Sample_x ~ Sample_y, scales='free')
figures[['clonal_abundance_scatter_large_TRA']]
```
...
tmp <- tmp %>%
  ...
  filter(Sample_x != Sample_y) %>%
  filter(max(Proportion_x, Proportion_y) > 0.001) ## minimum 0.1% of one repertoire
figures[['clonal_abundance_scatter_large_TRB']] <- ggplot(tmp, aes(x=Proportion_x, y=Proportion_y)) +
  geom_point(alpha=0.7, size=0.5) +
  # geom_density_2d_filled() +
  facet_grid(Sample_x ~ Sample_y, scales='free')
figures[['clonal_abundance_scatter_large_TRB']]
ggsave(here('figures/facet_grid_genes.png'), width = unit(15, 'in'), height = unit(15, 'in'))
```
